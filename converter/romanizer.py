from .substitution import LatynkaMapsa
...
class Romanizer:
    ...
    and set as default).
    """
    def __init__(self, text=None, rules=LatynkaMapsa()):
        self.rules = rules
        self.text = text
    ...
    def _resolve_ambiguity(self, word):
        ...
        # Since Python 3.7 upate dict elements are guaranteed to be ordered, thus further loops work ok.
        ...
        for amb in self.rules.AMBIGUOUS_CHAR_TAB:
            ...
            for char in self.rules.AMBIGUOUS_CHAR_TAB[amb]:
                if char in word:
                    word = getattr(self.rules, amb)(char, word)
        return word
